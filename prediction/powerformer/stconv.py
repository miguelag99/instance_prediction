...
from prediction.layers.spatial_temporal import Bottleneck, Residual
from torch import nn

conv_block = Residual # [Residual, Bottleneck]

class MultiBranchSTconv(nn.Module):
    """"""
    def __init__(
        self,
        cfg,
        ...
        print(f'Creating MultiBranchSTconv module with latent_dim={self.latent_dim}')
...
class STconv(torch.nn.Module):
    ...
    def forward(self, f_in):      
        ...
        # BEV Predictor
        f_dec = self.BEV_predictor(f_in)  # list of N x T_out x C_i x H_i x W_i
        # for state in f_dec:
        #     print(f'f_pred: {state.shape}')

        # BEV Decoder
        f_out = self.BEV_decoder(f_dec)  # N x T_out x C x H x W
        # for state in f_out:
        #     print(f'f_dec: {state.shape}')  
        # print('---------')
        # Output
        return f_out
